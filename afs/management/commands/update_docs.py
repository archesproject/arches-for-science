...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-s", "--source", default="node_descs.csv", action="store", dest="source", help="TSV file to read data from"
        )
        parser.add_argument(
            "-d", "--dest", default="node_descs.csv", action="store", dest="dest", help="TSV filename to write to"
        )
        parser.add_argument(
            "-o", "--operation", default="export", action="store", dest="op", help="One of: update, export"
        )
...
"""       
        ...
        with open(options['source']) as fh:
            ...
            for rec in reader:
                ...
                desc = template.format(spec=rec['Specific'],desc=rec['General'],
                    note=rec['Note'], bid=rec['BranchId'], fid=rec['FieldId'])
  ...
  "5bece219-c456-11e9-8dcd-a4d18cec433a": "2022f91c-c2c1-11ea-af13-3af9d3b32b71",  # Modification
    ...
    def align_branches(self, snode, tnode):
        ...
        for te in t_edges:
            ...
            if snid:
                ...
                for se in s_edges:
                    sn = se.rangenode
                    if se.ontologyproperty == te.ontologyproperty and \
                        sn.ontologyclass == tn.ontologyclass:
                        sdt = sn.datatype
                        tdt = tn.datatype
                        if sdt == tdt:
                            poss.append(sn)
    ...
    def handle_export(self, options):
        ...
        with open(options['dest'], 'w') as cfh:
            ...
            writer.writerow(("Graph", "NodeId", "Node Name", "Specific", "General",
                "Note", "BranchId", "FieldId", "Class", "Property", "DataType", "Count", "Path"))
            ...
            for graph in graphs:
                ...
                for node in nodes:
                    ...
                    if options['count'] and graph.isresource:
                        ...
                        c = -1
                    writer.writerow((gname, node.pk, name, dfields['specific'], dfields['general'],
                        dfields['note'], dfields['branchId'], dfields['fieldId'], 
                        self.make_readable(node.ontologyclass), prop, node.datatype, c, path))
