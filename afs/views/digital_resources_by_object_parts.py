...
class DigitalResourcesByObjectParts(View):
    def get(self, request, resourceid):
        ...
        digital_resource_tiles = TileModel.objects.filter(resourceinstance_id__in=physical_things).filter(nodegroup_id=digital_resource_nodegroup_id)
        related_datasets = {tile.data[digital_resource_node_id][0]["resourceId"] for tile in digital_resource_tiles}
        part_lookup = {tile.data[digital_resource_node_id][0]["resourceId"]: str(tile.resourceinstance_id) for tile in digital_resource_tiles}
        digital_resource_type_tiles = TileModel.objects.filter(resourceinstance_id__in=related_datasets).filter(nodegroup_id=digital_resource_type_nodegroup_id)
        manifest_datasets = {str(tile.resourceinstance_id) for tile in digital_resource_type_tiles if manifest_concept_value_id in tile.data[digital_resource_type_nodegroup_id]}
        digital_resources = Resource.objects.filter(pk__in=related_datasets - manifest_datasets)
        results = {"resources":[{"resourceid":str(resource.pk), "displayname":resource.displayname} for resource in digital_resources], "selected_resource": resourceid}
