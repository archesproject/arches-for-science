...
class SelectDatasetFilesStep(View):
    def post(self, request):
        physicalThingPartNameNodeId = "3e541cc6-859b-11ea-97eb-acde48001122"
        physicalThingPartAnnotationNodeId = "97c30c42-8594-11ea-97eb-acde48001122"
        physicalThingPartNodeId = "b240c366-8594-11ea-97eb-acde48001122"
        observationGraphId = "615b11ee-c457-11e9-910c-a4d18cec433a"
        digitalResourceGraphId = "707cbd78-ca7a-11e9-990b-a4d18cec433a"
        recordedValueNodeId = "dd596aae-c457-11e9-956b-a4d18cec433a"
        digitalReferenceNodeId = "a298ee52-8d59-11eb-a9c4-faffc265b501"
        digitalReferenceNodeGroupId = "8a4ad932-8d59-11eb-a9c4-faffc265b501"
        datasetNameNodeGroupId = "d2fdae3d-ca7a-11e9-ad84-a4d18cec433a"
        datasetNameNodeId = "d2fdc2fa-ca7a-11e9-8ffb-a4d18cec433a"
        datasetFileNodeGroupId = "7c486328-d380-11e9-b88e-a4d18cec433a"
        datasetFileNodeId = "7c486328-d380-11e9-b88e-a4d18cec433a"
        rendererLookup = {
        ...
        with transaction.atomic():
            ...
            if not dataset_resource_id:
                graphid = Node.objects.filter(nodegroup=datasetNameNodeGroupId)[0].graph_id
                ...
                dataset_name_tile = Tile().get_blank_tile_from_nodegroup_id(datasetNameNodeGroupId)
            ...
            dataset_name_tile.data[datasetNameNodeId] = dataset.get('name')
            ...
            digital_reference_tile = Tile().get_blank_tile_from_nodegroup_id(digitalReferenceNodeGroupId)
            digital_reference_tile.resourceinstance_id = part_resource_id
            digital_reference_tile.data[digitalReferenceNodeId] = [{
            ...
            if observation_ref_tile != None:
                ...
                relationships = observation_reference_tile.data[recordedValueNodeId]
                part_relationships = [item for item in relationships if item["partResource"] == part_resource_id]
                if len(part_relationships) == 0:
                    observation_reference_tile.data[recordedValueNodeId].append({
                        "resourceId": str(dataset_resource.resourceinstanceid),
                        "ontologyProperty": "",
                        "inverseOntologyProperty": "",
                ...
                observation_reference_tile = Tile().get_blank_tile_from_nodegroup_id(recordedValueNodeId)
                observation_reference_tile.resourceinstance_id = observation_id
                observation_reference_tile.data[recordedValueNodeId] = [{
                    "resourceId": str(dataset_resource.resourceinstanceid),
                    "ontologyProperty": "",
                    "inverseOntologyProperty": "",
                    "partResource": part_resource_id
            ...
            dataset_files = request.FILES.getlist("file-list_{}".format(datasetFileNodeId), None)
            file_data_list = request.POST.getlist("file-list_{}_data".format(datasetFileNodeId), None)
                        ...
                        for zip_file in files:
                            if not zip_file.filename.startswith("__MACOSX") and not zip_file.is_dir():
                                ...
                                new_files.append((file_data_copy, instrument, InMemoryUploadedFile(
                                    new_file_bytes,
                                    "file-list_{}".format(datasetFileNodeId),
                                    zip_file.filename,
                                    new_file_type,
                                    zip_file.file_size,
                                    None
            ...
            for file in new_files: 
                if file[0] is not None:
                    ...
                    tiles = (tile for tile in dataset_resource.tiles if tile.nodegroup_id == datasetFileNodeGroupId)
                    # delete/replace tiles for files that share the same name
                    for tile in tiles:
                        if tile.data[datasetFileNodeId][0]['name'] == file_data['name']:
                            tile.delete(request=request)


                    split_file_mime = file_data["type"].split('/')
                    if "image" in split_file_mime:
                        file_data["renderer"] = rendererLookup["image"]
                    elif "pdf" in split_file_mime:
                        file_data["renderer"] = rendererLookup["pdf"]
                    elif instrument != None: # instrument was given by zip file name
                        file_data["renderer"] = rendererLookup[instrument]

                    #file has not been uploaded
                    if not tile_id:
                        dataset_file_tile = Tile().get_blank_tile_from_nodegroup_id(datasetFileNodeGroupId)
                        dataset_file_tile.resourceinstance_id = str(dataset_resource.resourceinstanceid)
                        dataset_file_tile.data[datasetFileNodeId] = [file[0]]
                        ...
                        new_req.FILES["file-list_{}".format(datasetFileNodeId)] = file_content
                        new_tile = TileData()
