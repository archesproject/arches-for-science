define([
    ...
    function viewModel(params) {
        ...
        self.rendererConfigs = ko.observable();
        self.rendererUrl = ko.observable();
        self.renderers = ko.observable(Object.values(fileRenderers).map(format => {return {"text": format.name, "id": format.id};}));
        self.formats = ko.observableArray(Object.values(formats).map(format => {return {"text": format.name, "id": format.id};}));

        self.selectedFileFormat = ko.observable().extend({deferred: true, notify: 'always'});

        self.selectedConfig = ko.observable();
        self.processConfigs = (data) => {
            return { "results": data?.configs?.map(renderer => {return { text: renderer.name, "id": renderer.configid};})};
        };
        

        self.selectedFileFormat.subscribe(renderer => {
            if (!renderer) {
                return;
            }
            console.log(renderer); 
            self.rendererUrl(`/renderer/${self.selectedFileFormat()}`);
        });
        ...
        (async() => {
            ...
            self.files(self.digitalResources().flatMap(resource => {
