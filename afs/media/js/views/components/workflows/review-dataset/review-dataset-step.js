...
//     ko.components.register('review-dataset-step', {
//         viewModel: viewModel,
//         template: { require: 'text!templates/views/components/workflows/ew-tile-step.htmn' }
//     });
//     return viewModel;
// });
...
define([
    ...
    function viewModel(params) {
        ...
        var self = this;
        var digitalResourcesGraphId = '707cbd78-ca7a-11e9-990b-a4d18cec433a';

        var checkForDigitalResources = function(val){
            if (val) {
                var physicalThing = val["sample-object-resource-instance"][0][1][0].resourceId;
                // if (physicalThing) {
                //     resourceUtils.getRelatedInstances(physicalThing, digitalResourcesGraphId)
                //         .then(
                //             function(samples){
        ...
        checkForDigitalResources(params.value());
        params.value.subscribe(checkForDigitalResources);
    }

    ko.components.register('review-dataset-step', {
        viewModel: viewModel,
        template: {
            require: 'text!templates/views/components/workflows/review-dataset/review-dataset-step.htm'
        }
    });
    return viewModel;
});