define([
    ...
    function viewModel(params) {
        ...
        const removalFromObjectNodegroupId =
            "b11f217a-d2bc-11e9-8dfa-a4d18cec433a";
        const removedFromNodeId = "38814345-d2bd-11e9-b9d6-a4d18cec433a";
        const fileStatementContentNodeId =
            "ca227726-78ed-11ea-a33b-acde48001122";
            ...
            const count = self
                ...
                .reduce((acc, observation) => {
                    return (
                        ...
                        observation
                            .relatedFiles()
                            .filter((file) => file.selected()).length
                    );
                }, 0);
            return count;
        });
        ...
        this.toggleSelection = function () {
            ...
            self.relatedObservations().forEach((observation) => {
                observation
                    .relatedFiles()
                    .forEach((file) => file.selected(bool));
            });
            self.expandAll(true);
        };
            ...
            const projectPhysicalThings = collectionJson.related_resources
                ...
                .filter((res) => {
                    const removedFromTile = res.tiles.find(
                        (tile) =>
                            tile.nodegroup_id === removalFromObjectNodegroupId
                    );
                    const removedFrom = removedFromTile?.data[
                        removedFromNodeId
                    ].map((rr) => rr.resourceId);
                    return (
                        removedFrom?.some((res) =>
                            physicalThings.includes(res)
                            ...
                            if (tile.nodegroup_id == fileNodeId) {
                                ...
                                if (
                                    selectedFileIds.includes(
                                        tile.data[fileNodeId][0].file_id
                                    )
                                ) {
                                    selected(true);
                                }
                                ...
                                const interpretation = res.tiles.find(
                                    (tile2) =>
                                        tile2.parenttile_id == tile.tileid
                                )?.data[fileStatementContentNodeId][
                                    arches.activeLanguage
                                ].value;
                                const file = {
