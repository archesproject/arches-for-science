  ...
  function viewModel(params) {
    ...
    this.numberOfSelectedFiles = ko.computed(() => {
      const count = self.relatedObservations().reduce((acc, observation) => {
        return (
          acc +
          observation.relatedFiles().filter((file) => file.selected()).length
        );
      }, 0);
      return count;
    });
    ...
    this.getFilesFromObservation = async () => {
      const projectResponse = await window.fetch(
        arches.urls.related_resources + self.projectValue + '?paginate=false'
      );
      const projectJson = await projectResponse.json();

      const collectionForProject = projectJson.related_resources.find(
        (res) => res.graph_id === collectionGraphId
      ).resourceinstanceid;
      const collectionResponse = await window.fetch(
        arches.urls.related_resources + collectionForProject + '?paginate=false'
      );
      const collectionJson = await collectionResponse.json();

      const projectPhysicalThings = collectionJson.related_resources
        .filter((res) => res.graph_id === physicalThingGraphId)
        .filter((res) => {
          const removedFromTile = res.tiles.find(
            (tile) => tile.nodegroup_id === removalFromObjectNodegroupId
          );
          const removedFrom = removedFromTile?.data[removedFromNodeId].map(
            (rr) => rr.resourceId
          );
          return (
            removedFrom?.some((res) => physicalThings.includes(res)) ||
            physicalThings.includes(res.resourceinstanceid)
          );
        })
        .map((res) => res.resourceinstanceid);

      self.projectName = projectJson.resource_instance.displayname;
      const projectObservations = projectJson.related_resources
        .filter((res) => res.graph_id == observationGraphId)
        .filter((res) => {
          const objectTile = res.tiles.find(
            (tile) => tile.nodegroup_id === objectObservedNodeId
          );
          const object =
            objectTile?.data[objectObservedNodeId][0]['resourceId'];
      ...
      for (const projectObservation of projectObservations) {
        ...
        const response = await window.fetch(
          arches.urls.related_resources +
            projectObservation.resourceinstanceid +
            '?paginate=false'
        );
        ...
        if (response.ok) {
          ...
          observation.description =
            observation.descriptors[arches.activeLanguage].description;
          const digitalResources = json.related_resources.filter(
            (res) => res.graph_id == digitalResourcegGraphId
          );
          digitalResources.forEach((res) =>
            res.tiles.forEach((tile) => {
              if (tile.nodegroup_id == fileNodeId) {
                ...
                if (
                  selectedFileIds.includes(tile.data[fileNodeId][0].file_id)
                ) {
                  selected(true);
                }
                selected.subscribe(() => addSelectedFiles());
                const interpretation = res.tiles.find(
                  (tile2) => tile2.parenttile_id == tile.tileid
                )?.data[fileStatementContentNodeId][arches.activeLanguage]
                  .value;
                const file = {
                  ...tile.data[fileNodeId][0],
                  interpretation,
                  selected,
                };
          ...
          self.relatedObservations.push({
            ...observation,
            relatedFiles,
          });
        }
      }
      self.relatedObservations.sort(
        (a, b) => b.relatedFiles().length - a.relatedFiles().length
      );
    };
