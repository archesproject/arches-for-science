define([
    ...
    function viewModel(params) {
                ...
                res.then(
                    function(data){
                        ...
                        if (setTile && Object.keys(setTile.data).includes(self.physThingSetNodegroupId) && setTile.data[self.physThingSetNodegroupId].length) {
                            ...
                            let res = resourceUtils.lookupResourceInstanceDataByQuery(query);
                            res.then(
                                function(data) {
                                    physicalThingResourceIds = data.map((instance) => instance._source.resourceinstanceid)
                            ...
                            self.hasPhsyicalThingWithObservation(false);
                        }
                    }
                );
            }
        ...
        this.termFilter = ko.pureComputed(function(){
            if (ko.unwrap(self.physicalThingsThatBelongToSets)) {
                self.hasSetWithPhysicalThing(true);
                self.hasPhsyicalThingWithObservation(true);
                var query = {"op": "and"};
                query[self.observationPartOfPhysicalThingNodeId] = {
                    "op": "or",
                    "val":  ko.unwrap(self.physicalThingsThatBelongToSets)
                };
