...
import sampleTakingImageStepTemplate from 'templates/views/components/workflows/sample-taking-workflow/sample-taking-image-step.htm';
...
function viewModel(params) {
    ...
    this.saveSamplingActivityDigitalReference = function(digitalResourceInstanceId){
        ...
        return window.fetch(generateArchesURL("arches:api_tiles", {tileid: self.samplingActivityDigitalReferenceTileId}), {
            method: 'POST',
            credentials: 'include',
    ...
    this.save = async function() {
        ...
        if (label === self.selectedPhysicalThingImageServiceName()) {
            ...
            const response = await fetch(`${generateArchesURL("arches_for_science:manifest_x_canvas")}?manifest=${manifestId}`);
            const data = await response.json();
            const digitalResourcesResourceId = data.digital_resource;

            const card_response = await fetch(generateArchesURL("arches:api_card", { resourceid: digitalResourcesResourceId }).urls.api_card);
