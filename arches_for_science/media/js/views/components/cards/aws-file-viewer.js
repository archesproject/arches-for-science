...
export default ko.components.register('aws-file-viewer-card', {
    viewModel: function(params) {
        ...
        this.addTile = function(file){
            const newtile = self.card.getNewTile();
            const defaultRenderers = self.getDefaultRenderers(file.type, file.name);
            const tilevalue = {
                name: file.name,
                accepted: true,
        ...
        this.uppyOptions = {
            ...
            autoProceed: true,
            filesAdded: (files) => {
                files.successful.map(file => self.addTile(file));
            }
        };
    },
